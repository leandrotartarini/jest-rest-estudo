...
describe('Ao calcular o saldo do usuário', () => {
  ...
  test('Deve subtrair as contas de saida', async () => {
    await request(app).post(ROUTE_TRANSACTION)
  ...
  test('Não deve considerar saldos de contas distintas', async () => {
    await request(app).post(ROUTE_TRANSACTION)
...
test('Deve calcular saldo das contas do usuário', async () => {
  const res = await request(app).get(MAIN_ROUTE).set('authorization', `bearer ${TOKEN_GERAL}`);
  expect(res.status).toBe(200);
  expect(res.body).toHaveLength(2);
