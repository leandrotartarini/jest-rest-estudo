...
test('Deve listar apenas as transferencias do usuário', async () => {
  const res = await request(app).get(MAIN_ROUTE).set('authorization', `bearer ${TOKEN}`);
...
test('Deve inserir uma transferência com sucesso', async () => {
  const res = await request(app).post(MAIN_ROUTE).set('authorization', `bearer ${TOKEN}`)
    .send({
      description: 'Regular Transfer', user_id: 10000, acc_ori_id: 10000, acc_dest_id: 10001,
      ammount: 100, date: new Date()
    });
...
describe('Ao tentar salvar uma transferência inválida...', () => {
  ...
  test('Não deve inserir se as contas pertencerem a outra usuário', () => template({ acc_ori_id: 10002 }, 'Conta #10002 não pertence ao usuário'));
});

test('Deve retornar uma transferência por ID', async () => {
  const res = await request(app).get(`${MAIN_ROUTE}/10000`).set('authorization', `bearer ${TOKEN}`)
  expect(res.status).toBe(200);
...
describe('Ao alterar uma transferência válida...', () => {
  ...
  test('Deve retornar o status 200 e os dados da transferência', async () => {
    const res = await request(app).put(`${MAIN_ROUTE}/10000`).set('authorization', `bearer ${TOKEN}`)
      .send({
        description: 'Transfer Updated', user_id: 10000, acc_ori_id: 10000, acc_dest_id: 10001,
        ammount: 500, date: new Date()
      });
    expect(res.status).toBe(200);
...
describe('Ao tentar alterar uma transferência inválida...', () => {
  ...
  const template = async (newData, errorMessage) => {
    const res = await request(app).put(`${MAIN_ROUTE}/10000`)
      .set('authorization', `bearer ${TOKEN}`)
      .send({ ...validTransfer, ...newData });
    expect(res.status).toBe(400);
    expect(res.body.error).toBe(errorMessage);
...
describe('Ao remover uma transferência', () => {
  test('Deve retornar o status 204', async () => {
    const res = await request(app).delete(`${MAIN_ROUTE}/10000`)
      .set('authorization', `bearer ${TOKEN}`)
...
test('Não deve retornar transferências de outro usuário', async () => {
  const res = await request(app).get(`${MAIN_ROUTE}/10001`)
    .set('authorization', `bearer ${TOKEN}`)
